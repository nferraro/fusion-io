...
bool trace_integrator::open_path_file(const char* filename) {
  if(path_file.is_open()) close_path_file();
  path_file.open(filename, std::fstream::out | std::fstream::trunc);
  return path_file.is_open();
}

bool trace_integrator::close_path_file() {
...
bool trace_integrator::integrate_adaptive(int transits, int steps_per_transit, integrator_data* data)
{
    ...
    while (transits_count < transits) {
        ...
        // NEW: Log the 3D trajectory regardless of whether we hit a puncture plane
        // Format: Phi, R, Z (Full 3D coordinate set)
        if (path_file.is_open()) {
            path_file << std::setiosflags(std::ios::scientific) << std::setw(20) 
                      << Phi << std::setw(20) << R << std::setw(20) << Z << std::endl;
        }
...
void trace_integrator::save_puncture(double pl, double R_p, double Z_p, double Z0, double R0, double lyap) {
    ...
    file << std::setiosflags(std::ios::scientific) << std::setw(20) << std::setprecision(12) 
         ...
         << std::setw(20) << theta_plot << std::setw(20) << psi_plot;
    
    // --- NEW: Add the 6th column if it's a Lyapunov run ---
    if (!std::isnan(lyap)) {
         file << std::setw(20) << lyap;
    }
...
// --- FAST VARIATIONAL EQUATIONS: TIME-AVERAGED MEGNO ---
bool trace_integrator::integrate_lyapunov(int transits, int steps_per_transit, double& lyap_val)
{
    // The Exact State Vector:
    // [0]=R, [1]=Z                 (Base Particle)
    // [2]=u_R, [3]=u_Z             (Tangent Vector)
    // [4]=W                        (MEGNO Integral)
    // [5]=I                        (Time-Averaged MEGNO Accumulator)
    double y[6] = {R, Z, 1.0, 0.0, 0.0, 0.0}; 
    double p_cur = Phi;
    const double phi_init = Phi;

    double dphi = (reverse ? -period : period) / (double)steps_per_transit;
    int steps = transits * steps_per_transit;
    int valid_steps = 0;

    // Fast RK4 integrating the base trajectory, tangent vector, and MEGNO simultaneously
    auto rk4_step = [&](double h) -> bool {
        double k1[6], k2[6], k3[6], k4[6];
        double y_tmp[6];

        auto deriv = [&](double p, const double y_in[6], double dy_out[6]) {
            double r = y_in[0], z = y_in[1], ur = y_in[2], uz = y_in[3];

            // 1. Base Field Evaluation
            double br, bphi, bz;
            if (!eval(r, p, z, &br, &bphi, &bz)) return false;
            if (std::abs(bphi) < 1e-12) return false;
            double factor = (toroidal ? r : 1.0) / bphi;
            double fR = br * factor;
            double fZ = bz * factor;
            dy_out[0] = fR;
            dy_out[1] = fZ;

            // 2. Jacobian via Efficient Forward Difference (Saves massive computation time)
            double eps = 1e-6;
            double br_r, bphi_r, bz_r;
            if (!eval(r+eps, p, z, &br_r, &bphi_r, &bz_r)) return false;
            double fR_r = br_r * (toroidal ? (r+eps) : 1.0) / bphi_r;
            double fZ_r = bz_r * (toroidal ? (r+eps) : 1.0) / bphi_r;
            double J11 = (fR_r - fR) / eps;
            double J21 = (fZ_r - fZ) / eps;

            double br_z, bphi_z, bz_z;
            if (!eval(r, p, z+eps, &br_z, &bphi_z, &bz_z)) return false;
            double fR_z = br_z * (toroidal ? r : 1.0) / bphi_z;
            double fZ_z = bz_z * (toroidal ? r : 1.0) / bphi_z;
            double J12 = (fR_z - fR) / eps;
            double J22 = (fZ_z - fZ) / eps;

            // 3. Exact Tangent Vector Evolution (J * u)
            dy_out[2] = J11 * ur + J12 * uz;
            dy_out[3] = J21 * ur + J22 * uz;

            // 4. MEGNO Integration
            double t = std::abs(p - phi_init);
            double dt_dp = reverse ? -1.0 : 1.0;
            double norm2 = ur*ur + uz*uz;
            
            if (norm2 > 1e-30) {
                double growth_rate = (ur * dy_out[2] + uz * dy_out[3]) / norm2; // d(ln|u|)/dp
                dy_out[4] = t * growth_rate; // dW/dp
            } else {
                dy_out[4] = 0.0;
            }

            if (t > 1e-12) {
                dy_out[5] = (2.0 * y_in[4] / t) * dt_dp; // d(Integral)/dp = Y(t) * dt/dp
            } else {
                dy_out[5] = 0.0;
            }
        ...
        // Execute RK4
        if (!deriv(p_cur, y, k1)) return false;
        for(int i=0; i<6; ++i) y_tmp[i] = y[i] + h*0.5*k1[i];

        if (!deriv(p_cur + h*0.5, y_tmp, k2)) return false;
        for(int i=0; i<6; ++i) y_tmp[i] = y[i] + h*0.5*k2[i];

        if (!deriv(p_cur + h*0.5, y_tmp, k3)) return false;
        for(int i=0; i<6; ++i) y_tmp[i] = y[i] + h*k3[i];

        if (!deriv(p_cur + h, y_tmp, k4)) return false;
        ...
        // Continuous Mathematical Renormalization
        // Because the MEGNO equation uses (u * du/dt)/|u|^2, it is strictly invariant to length.
        // Forcing length to 1.0 at every step flawlessly prevents stellarator geometric overflow!
        double n = std::sqrt(y[2]*y[2] + y[3]*y[3]);
        if (n > 1e-20) {
            y[2] /= n;
            y[3] /= n;
        } else {
    ...
    double next_puncture = plane;
    if ((!reverse && p_cur >= plane) || (reverse && p_cur <= plane)) {
         next_puncture += (reverse ? -period : period);
    }

    for (int step = 1; step <= steps; ++step) {
        
        if (!rk4_step(dphi)) break;

        bool landing = false;
        if ((!reverse && p_cur >= next_puncture) || (reverse && p_cur <= next_puncture)) {
            landing = true;
        }

        // --- DIRECT WRITE TO out* FILES ---
        if (landing) {
            double t = std::abs(p_cur - phi_init);
            
            // Time-Averaged MEGNO: <Y> = Integral / t
            double current_mean_megno = (t > 0) ? (y[5] / t) : 0.0;
            save_puncture(next_puncture, y[0], y[1], Z0_mag, R0_mag, current_mean_megno);
    ...
    if (valid_steps == 0) return false;

    double t = std::abs(p_cur - phi_init);
    lyap_val = (t > 0) ? (y[5] / t) : NAN;
    
    R = y[0]; Z = y[1]; Phi = p_cur;
    return true;
}