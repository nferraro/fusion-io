...
int main(int argc, char* argv[])
{
  ...
  if(pout) {
    ...
    for(int j=0; j<surfaces; j++) {
      ...
      if(j>0) plotfile << ",\\\n";
      plotfile << "'" << outfile << "'" << " using 2:3 title '' lc rgb 'black'";
    }
    plotfile << std::endl;
    plotfile.close();
  ...
  for(int j=0; j<local_surfaces[rank]; j++) {
    ...
    trace_integrator_s::integrator_data data;
    my_q_mean[j] = 0.;
    my_q_max[j] = 0.;
    my_q_min[j] = 0.;
    int tt = 0;
...
bool create_source(const int type, const int argc, const std::string argv[]) 
{
  ...
  if(result != FIO_SUCCESS) {
    std::cerr << "Error reading rst" << std::endl;
    src.free();
    return false;
  };
  result = src.source->get_field(FIO_ZSTELLA, &src.zst, &fopt);
  if(result != FIO_SUCCESS) {
    std::cerr << "Error reading zst" << std::endl;
    src.free();
    return false;
  };
